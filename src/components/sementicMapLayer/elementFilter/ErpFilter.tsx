...
const ErpFilter = ({ editorOpen, setEditorOpen }: ErpFilterProps) => {
  ...
  const resetErpList = useResetRecoilState(resetErp);
  ...
  const [localModalIdx, setLocalModalIdx] = useState<number>(0);
    ...
    getBsDisList(tmp)
      .then((res) => {
        ...
        if (data && data.length > 0) {
          ...
          //   if (li.polygonType !== "single" && li.polygonType !== "multi") {
          //     console.log(li);
          //   }
          ...
          const makeCenter = data.map((li: any) => {
            if (li.center) {
              const center = li.center.coordinates;

              if (!li.polygonType && li.range && li.center) {
                return { ...li, center: center, polygonType: "circle" };
              } else {
